...
export default async function createLintSingleFile(importAsESLint, engine)
{
    ...
    const [{ createIgnoreResult }, { WarningService }, { default: createDebug }, verifyText] =
    await Promise.all
    (
        [
            importAsESLint('./lib/eslint/eslint-helpers.js'),
            importAsESLint('./lib/services/warning-service.js'),
    ...
    async function loadConfigArrayForFile(filePath)
    {
        ...
        const { emitEmptyConfigWarning } = warningServicePrototype;
        warningServicePrototype.emitEmptyConfigWarning = () => {};
        try
        {
        ...
        {
            warningServicePrototype.emitEmptyConfigWarning = emitEmptyConfigWarning;
        }
    }
